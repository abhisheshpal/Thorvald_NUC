...
#include <thorvald_2d_nav/control_commands.h>
...
thorvald_2d_nav::control_commands curr_state;
...
{
DRIVE_MODE_1 = 1, DRIVE_MODE_2 = 2, DRIVE_MODE_3 = 3,
NUMBER_OF_CONTROL_MODES = 3
};

void commandsCallback (const thorvald_2d_nav::control_commands state_msg)
{
curr_state = state_msg;
}
...
//left row_detection
float left_row_detection(sensor_msgs::LaserScan left_scan, float func_final_x_1, float func_final_y_1){

int initial_count_1 = 0;
for (int i = 0; i <= 90; i++){
 angle[i] = left_scan.angle_min+i*left_scan.angle_increment;
 x[i] = left_scan.ranges[i]*cos(angle[i]);
 y[i] = left_scan.ranges[i]*sin(angle[i]);

if(!std::isnan(x[i]) && !std::isnan(y[i]) && (k == 0) && (left_scan.ranges[i] < min_range)){
 range_1[initial_count_1] = left_scan.ranges[i];
...
func_final_x_1 = sum_x_1/(k-1);
func_final_y_1 = sum_y_1/(k-1);
k = 1;
         ...
         marker_1.pose.position.x = func_final_x_1;
         marker_1.pose.position.y = func_final_y_1;
...
//right row_detection
float right_row_detection(sensor_msgs::LaserScan right_scan, float func_final_x_2, float func_final_y_2){
              
int initial_count_2 = 0;

for (int i = 270; i <= 360; i++){
 angle[i] = right_scan.angle_min+i*right_scan.angle_increment;
 x[i] = right_scan.ranges[i]*cos(angle[i]);
 y[i] = right_scan.ranges[i]*sin(angle[i]);

if(!std::isnan(x[i]) && !std::isnan(y[i]) && (q == 0) && (right_scan.ranges[i] < min_range)){
 range_2[initial_count_2] = right_scan.ranges[i];
...
func_final_x_2 = sum_x_2/(q-1);
func_final_y_2 = sum_y_2/(q-1);
q = 1;
         ...
         marker_2.pose.position.x = func_final_x_2;
         marker_2.pose.position.y = func_final_y_2;
...
int main(int argc, char** argv) 
{
	...
	ros::Subscriber commands_sub = n.subscribe("current_state", 100, commandsCallback);
	...
	while (ros::ok()){
        ...
        if(curr_state.drive_mode == 1){
        ControlDriveModes current_drive_mode = ControlDriveModes::DRIVE_MODE_1;
        } 

        // Row detection Function
        left_row_detection(scan_msg_main, final_x_1, final_y_1);
        right_row_detection(scan_msg_main, final_x_2, final_y_2);
